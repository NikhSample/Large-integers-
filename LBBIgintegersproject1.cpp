...
class BigInt{
	string digits;			// Assuming digits are stored in a string
public:

	//Constructors:
	BigInt(unsigned long long n = 0);
    BigInt(const char *);	//for string literals
	BigInt(string &);		//for string
...
BigInt::BigInt(string & s){		// Constructor that takes a 
	digits = "";
	int n = s.size();
	for (int i = n - 1; i >= 0;i--){
		if(!isdigit(s[i]))
...
BigInt::BigInt(unsigned long long nr){		//Constructor that takes an int/long long int
	do{
		digits.push_back(nr % 10);
		nr /= 10;
	} while (nr);
}
BigInt::BigInt(const char *s){				// Constructor that takes a const char*
	digits = "";
...
BigInt::BigInt( BigInt & a){				//copy constructor 	
	digits = a.digits;
}
...
int main()
{
	BigInt first("98765");				//"12345" is a string literal and is of type const char[6] (including the null terminator \0). uses const char* constructor
										/*string str = "12345";
    									  BigInt first(str);  // This uses the string constructor*/
	cout << "The number of digits"
		<< " in first big integer = "
		<< Length(first) << '\n';
	BigInt second(98765);				//use int/long long int constructor- line 81
	if (first == second) {				//line 117
